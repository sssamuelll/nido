...
**Resolution path (mechanical):** lift-and-shift the GET / POST / DELETE handlers from `server/index.ts` into a new `server/routes/categories.ts` exporting a default `Router`; mount with `app.use('/api/categories', authenticateToken, apiLimiter, categoriesRouter)`. Move the `categoryUpsertSchema, CategoryUpsertInput, validate` imports out of `server/index.ts` into the new file. After the extract, the supertest fixture in `server/categories-wiring.test.ts` (added by Eje K) can be deleted in favor of a `getRouteMiddleware` test in a new `server/routes/categories.test.ts` matching the events test pattern.
